(function($, undefined){
	...
	*/
	...
	var appendFieldManager = new acf.Model({
		...
		actions: {
			'initialize_field' : 'initializeField'
		},
...
(function($, undefined){
	...
	acf.FieldObject = acf.Model.extend({
		...
		onChanged: function( e, name, value ){
			...
			// save meta
			if( name == 'menu_order' || name == 'parent' ) {
				this.save('meta');
			...
			// action for 3rd party customization
			acf.doAction('change_field_object_' + name, this);
		},
...
(function($, undefined){
	...
	*/
	...
	var settingsManager = new acf.Model({
		...
		actions: {
			//'initialize_field_object':	'initializeFieldObject',
			'initialize_field':			'initializeField'
		},
		
		//initializeFieldObject: function( fieldObject ) {
		//	console.log('initializeFieldObject');
		//	this.fieldObject = fieldObject;
		//},
		
		initializeField: function( field ){
			//console.log('initializeField');
			var setting = acf.newFieldSetting( field );
		}
	...
	*/
	...
	acf.FieldSetting = acf.Model.extend({
		...
		setup: function( field ){
			this.field = field;
			this.$el = field.$el;
			this.fieldObject = acf.getFieldObject(field.closest('.acf-field-object'));
		},
		...
		siblings: function( name ){
			return this.$el.siblings('[data-name="' + name + '"]');
		}
	});
	...
	var ConditionalLogicFieldSetting = acf.FieldSetting.extend({
		...
		renderField: function(){
			...
			if( !choices.length ) {
				choices.push({
					id: '',
					text: acf._e('no_conditions_available')
				});
			}
...
(function($, undefined){
	...
	*/
	...
	acf.newFieldObject = function( $field ){
		...
		// action
		acf.doAction('initialize_field_object', field);
	...
	*/
	...
	var allFields = new acf.Model({
		actions: {
			'append_field_object': 	'reset',
			'remove_field_object': 	'reset'
		},
		reset: function(){
	...
	*/
	...
	var fieldManager = new acf.Model({
		...
		onDuplicateField: function( field, newField ){
			...
			// vars
			var parentKey = newField.prop('key');
			
			// refresh children
			newField.getChildren().map(function( child ){
				child.wipe();
				child.prop('parent', parentKey);
				child.save();
			});
	...
	*/
	...
	var actionManager = new acf.Model({
		...
		actions: {
			...
			'sortstop_field_object':		'onSortstopFieldObject'
		},
		...
		onSortstopFieldObject: function( field ){
